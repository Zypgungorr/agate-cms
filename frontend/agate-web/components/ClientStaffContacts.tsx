...
interface ClientStaffContactsProps {
  clientId: string;
}

export default function ClientStaffContacts({ clientId }: ClientStaffContactsProps) {
  ...
  const loadStaff = async () => {
    try {
      const [clientStaffData, allStaffData] = await Promise.all([
        getClientStaff(clientId),
        getStaffList(false)
      ]);
      setStaff(clientStaffData);
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      await assignStaffToClient(clientId, {
  ...
  const handleRemoveStaff = async (staffId: string) => {
    ...
    if (!member || !confirm(`Remove ${member.fullName} as contact for this client?`)) return;

    try {
      await removeStaffFromClient(clientId, staffId);
      await loadStaff();
    } catch (err) {
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1 ml-6">
                Primary contacts are the main point of contact for this client
              </p>
            </div>
      ...
      {/* Staff List */}
      {staff.length === 0 ? (
        <p className="text-gray-500 text-sm">No contact persons assigned yet.</p>
      ) : (
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {/* Note: isPrimary info is not in StaffListItem, we'd need to enhance the API response */}
                </div>
